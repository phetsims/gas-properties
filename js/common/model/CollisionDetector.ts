...
export default class CollisionDetector {
  ...
  protected updateParticleContainerCollisions(): number {
    ...
    for ( let i = this.particleArrays.length - 1; i >= 0; i-- ) {
      numberOfParticleContainerCollisions += CollisionDetector.doParticleContainerCollisions( this.particleArrays[ i ],
        this.container.bounds, this.container.getLeftWallVelocityX() );
    }
   ...
   * @param leftWallVelocityX - velocity x-component of the container's left (movable) wall
   * @returns number of collisions
   */
  protected static doParticleContainerCollisions( particles: Particle[], containerBounds: Bounds2, leftWallVelocityX = 0 ): number {
    ...
    for ( let i = particles.length - 1; i >= 0; i-- ) {
      ...
      if ( particle.left <= containerBounds.minX ) {
        ...
        particle.setVelocityXY( -( particle.vx - leftWallVelocityX ), particle.vy );
        collided = true;
      }
